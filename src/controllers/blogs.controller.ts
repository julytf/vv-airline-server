...
export default {
  getAllPaginate: catchPromise(async function (req, res, next) {
    ...
    const query = new AdjustQuery(Blog.find())
    ...
    const count = await new AdjustQuery(Blog.find()).nameFilter(q as string).query.countDocuments()
  ...
  getOne: factory.getOne(Blog),
  createOne: factory.createOne(Blog),
  updateOne: factory.updateOne(Blog),
}
